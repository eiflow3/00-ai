...
From `CLAUDE.md`, and enforced by review rather than tooling:

- Layer dependencies one way only: **routers → services → schemas → config**.
- Routers stay HTTP-only. Anything a router would reach for as a helper belongs
  in `services/`, one module per responsibility.
- Services stay framework-agnostic: no FastAPI imports, callable without a
  request.
- Every request, response and streamed event payload is a Pydantic model in
  `schemas/`.
- An LLM provider is an adapter plus a factory registry entry — never an
  `if`-branch at a call site. Vendor responses are normalised at the service
  boundary.
- A stored file is linked to its vectors only through `services/provenance.py`.
  No call site spells out an id format or a metadata key.
- Synchronous SDK clients are wrapped in `asyncio.to_thread`, so they never
  block the event loop.
- When a non-essential stage fails mid-request, emit an error event and keep
  streaming.
- OpenAPI docs live in `docs/`. Adding or changing a streamed event means
  updating its schema **and** its docs module in the same change.
- Never do long-running work inside a response: a request starts a job and
  returns its id, and progress streams from an endpoint any client can reopen.
- Derive progress from what the stores already hold rather than a job table.
- Log every pipeline stage and record every run; a run that leaves no trace
...
**Claude is listed but unavailable.** The key is probably identity-linked —
issued to a person rather than a workspace. Set `ANTHROPIC_WORKSPACE_ID`.
`GET /chat/models` reports the reason per option.

**`stale_model` on every row.** `embedding_model` in `config.py` changed. Scores
across two embedding spaces are meaningless, so retrieval refuses rather than
returning confident nonsense. Re-index.

**A row is stuck on `interrupted`.** A previous run stopped partway. Re-index
it — only the missing chunks are embedded.

**Chunk count shows `8/0`.** The file was indexed before `chunk_total` was
recorded, so completeness cannot be judged. It reads `current` correctly and
picks the total up on its next re-index.

**A dev-server reload killed a run.** `--reload` restarts the process on every
file save, and a run lives in that process. Expected; the run is recorded as
`abandoned` and re-indexing resumes it.

**Log lines appear twice.** Two servers are running from the same directory and
sharing one log file. The rotating handler is not multi-process safe.

**Retrieval returns nothing.** The index may genuinely be empty — check the
Sources tab for a row reading `current`. A newly written vector also takes a
moment to become searchable; Pinecone is eventually consistent.

**Uploads refused.** Only `.txt`, `.md` and `.markdown` are accepted, non-empty
and under 10 MB.
...
- **Single process by design.** The queue, the in-flight marker and the live
  event buffer are in memory, which is correct for one process and wrong for
  two. A second worker or instance moves them to Redis; nothing else about the
  design changes. Redis Streams would also replace the hand-written cursor
  buffer, which is the part currently doing a broker's job.
- **A run dies with its process.** The queue does not survive a restart, so
  interrupted work needs a fresh Index click — cheap, because runs resume.
- **`GET /sources` is not cached** and does more work than it needs to: it walks
  the whole index, then re-reads each file's provenance one at a time — data the
  walk already had. Roughly 26 Pinecone calls for 10 files where 6 would do.
- **No delete-the-file endpoint.** Only a file's vectors can be removed through
  the app.
- **`evals/run_eval.py` predictions export is not wired up.** It needs a way to
  map a recorded answer back to a golden question id.
- **Text formats only.** PDF and DOCX would each be one extractor and its
  dependency.
- **No automated test suite in the repository.** Verification has been
  integration testing against the live services, which means tests need care
  around Pinecone's eventual consistency: an assertion that reads straight after
  a write tests propagation rather than logic.
