...
*   **Up to 5 serverless indexes** on the Starter plan, plus assistants. (This
    note previously said *one* index, which is wrong and nearly cost this
    project a design — see [06-pinecone-index-isolation.md](06-pinecone-index-isolation.md).
    Namespaces inside an index are effectively unlimited, so the index count is
    rarely the constraint it looks like.)
...
*   **Inactivity Limits:** If you don't use your free index for a few days, Pinecone might automatically archive or delete it to save resources, requiring you to rebuild it. Worth knowing before you leave four chunking experiments sitting untouched for a fortnight.
