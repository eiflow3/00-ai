...
async def list_indexed_source_keys() -> set[str]:
    ...
    # One vector per document is enough to recover its source key, and each
    # document's ids share a prefix — so keep the first id of each document
    # rather than fetching metadata for the entire index.
    ...
    return {
        ...
        for key in [_metadata_of(record).get(METADATA_SOURCE_KEY)]
        if key
    }
